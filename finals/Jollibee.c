...
void addMealToOrder(MealOrder* MO, Menu menuList, Meal newOrder)
{
	...
	if(i < MENUSIZE){
		...
		for(trav = MO->ML; trav != NULL; trav = trav->link){
			MO->bill += trav->qty * trav->order.price;
		}
...
void displayMealOrder(MealOrder MO)
{
	...
	MealList trav;
	for(trav = MO.ML; trav != NULL; trav = trav->link){
		printf("\n%-10s%-35s%-10d%-10d%-20.2f%-20.2f%-10s", MO.ML->order.mealCode, MO.ML->order.mealName, MO.ML->qty, MO.ML->delQty, MO.ML->order.price, MO.ML->qty * MO.ML->order.price, (MO.ordStat == 0) ? "NO" : "YES");
	}
