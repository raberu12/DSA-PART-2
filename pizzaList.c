...
typedef struct {
	int count;
	int orderStack[MAXORDERS];
}stackOrder;

typedef struct {
	char customerID[9];
	nameDetails custName;
	stackOrder custOrder;
} customer;
...
void addNewCustomer(cDict customers, customer C) {
	int hashVal = hashCustomer(C.customerID);
	custList temp, *trav;
	temp = (custList)malloc(sizeof(customerNode));

	if(temp != NULL) {
		for(trav = &customers[hashVal]; *trav != NULL && strcmp(C.customerID, (*trav)->C.customerID) != 0, strcmp(C.custName.fName, (*trav)->C.custName.fName) > 0; trav = &(*trav)->nextCust) {}
		if(strcmp(C.customerID, (*trav)->C.customerID) != 0) {
			temp->C = C;
			temp->nextCust = *trav;
			*trav = temp;
		} else {
			printf("That customer already exists!");
		}
	}
}
...
void populateCustomers(cDict customers) {
	...
	if(fp != NULL) {
		while(fread(&buffer, sizeof(customer), 1, fp)) {
			addNewCustomer(customers, buffer);
		}
	}
	fclose(fp);
}
...
pizzaType buildPizza(char pizzaName[], toppingsCW toppings) {
	...
	for(mask = 1 << (sizeof(toppingsCW)*8-1), i = 0; mask > 0, i < 8; mask >>= 1, i++) {
		if(toppings & mask == 1) {
...
void addOrder(orderList *orders, cDict customers, char customerID){
...
void orderPizza(orderList* orders, cDict customers, char customerID[], pizzaType pizza) {
	...
	for(i = 0; i < MAXORDERS; i++) {
		orders->orderList[i].stat = OPEN;
		orders->orderList[i].pList = NULL;
		orders->orderList[i].orderNumber = lastOrderNumber;
		strcpy(orders->orderList[i].cust.customerID, "XXXXXXX");
		strcpy(orders->orderList[i].cust.custName.fName, "XXXXXXX");
		strcpy(orders->orderList[i].cust.custName.lName, "XXXXXXX");
	}
