...
int main( ) {
	...
	*---------------------------------------------------------------------------------------*/
	...
	insert(&root, &vheap, prod01);
	insert(&newRoot, &vheap, prod02);
	insert(&newRoot, &vheap, prod03);
	insert(&newRoot, &vheap, prod04);
	printf("\n\nElements of BST new");  //Uncomment and put before the call to displayBSTAscending()
	displayBSTAscending(newRoot, vheap);
...
void displayVHeap(VHeap V) {
	...
	int i;
	for(i = 0; i < VH_SIZE; i++) {
		printf("\n%10d%10s%15d%15d", i, V.VH_node[i].elem.prodID, V.VH_node[i].LC, V.VH_node[i].RC);
	}
...
void displayHeap(minHeap HL) {
	...
	for(x = 0; x <= HL.lastNdx; x++) {
		printf("\n%-8s%-10s%-5d%-10.2f%-10d", HL.prod[x].prodID, HL.prod[x].prodDesc.name, HL.prod[x].prodDesc.weight, HL.prod[x].prodPrice, HL.prod[x].prodQty);
	}
}
...
void heapifySubtree(minHeap *H, int subroot) {
	//Write your code here!!!
	int currentNdx;
	for(currentNdx = subroot; currentNdx <= H->lastNdx;) {
		int lc = currentNdx * 2 + 1;
		int rc = lc + 1;
		if(lc <= H->lastNdx) {
			int smaller = lc;
			if(rc <= H->lastNdx && strcmp(H->prod[rc].prodID, H->prod[smaller].prodID) < 0) {
				smaller = rc;
			}
			if(strcmp(H->prod[currentNdx].prodID, H->prod[smaller].prodID) > 0) {
				swapProduct(&H->prod[currentNdx], &H->prod[smaller]);
				currentNdx = smaller;
			}else{
