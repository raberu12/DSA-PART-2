...
MST kruskalMST(Graph G) {
	...
	for(i = 0; i < edgeCounter; i++){
		for(j = 0; j < edgeCounter-i-1; j++){
			if(sortEdge[j+1].weight < sortEdge[j].weight){
	...
	for(i = 0; i < edgeCounter && retval.edgeCount < V - 1; i++){
		...
		while(disjoint[head] != head){
			head = disjoint[head];
		}
		while(disjoint[tail] != tail){
			tail = disjoint[tail];
		}
		if(head != tail){
			retval.edges[retval.edgeCount].head = head;
...
int *dijkstraPath(Graph G, int startVertex) {
	...
	for(i = 0; i < V; i++){
		...
		visited[minIndex] = 1;
		for(v = 0; v < V; v++){
			if(!visited[v] && retval[minIndex] != INF && G[minIndex][v] != INF && G[minIndex][v] + retval[minIndex] < retval[v]){
				retval[v] = G[minIndex][v] + retval[minIndex];
			}
		}
	}
	return retval;
...
void printDijkstra(int *D) {
	...
	for(i = 0; i < V; i++) {
		printf("\n%5d", i);
		D[i] != INF ? printf("%5d", D[i]) : printf("%5s", "X");
	}
	printf("\n");
}
