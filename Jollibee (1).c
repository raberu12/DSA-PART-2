...
Customer* readCustRecords(void)
{
	...
	Customer *custList = (Customer*)malloc(sizeof(Customer) * CUSTSIZE);

	if(fp != NULL){
		fread(custList, sizeof(Customer), CUSTSIZE, fp);
	}
...
{
	MealOrder new = {NULL, 0, cust, 0.00, -1};
	return new;
}
...
void addMealToOrder(MealOrder* MO, Menu menuList, Meal newOrder)
{
	...
	if(strcmp(menuList.menuItems[i].mealCode, newOrder.mealCode) == 0){

		for(temp = &MO->ML; *temp != NULL && strcmp((*temp)->order.mealName, newOrder.mealName) != 0; temp = &(*temp)->link){}
		if(*temp == NULL){
...
void displayMealOrder(MealOrder MO)
{
	...
	for(trav = MO.ML; trav != NULL; trav = trav->link){
		printf("\n%-10s%-35s%-10d%-10d%-20.2f%-20.2f%-10s", trav->order.mealCode, trav->order.mealName, trav->qty, trav->delQty, trav->order.price, trav->qty * trav->order.price, (trav->mealStat == 0) ? "NO" : "YES");
	}
...
{
	int i, allDelivered = 1;
	MealList trav;

	for(i = 0; i < numMeals; i++){
		for(trav = MO->ML; trav != NULL && strcmp(trav->order.mealName, DList[i].mealName) != 0; trav = trav->link){}
		...
		if(trav != NULL){
			if(trav->delQty < trav->qty){
				...
				printf("%s has already been delivered", trav->order.mealName);
			}

		}else{
			printf("You didn't order a %s.", DList[i].mealName);
		}
	}
...
void removeMealFromOrder(MealOrder* MO, Meal cancelMeal)
{
	...
	for(trav = &MO->ML; *trav != NULL && strcmp((*trav)->order.mealName, cancelMeal.mealName) != 0; trav = &(*trav)->link){}
	...
	if(*trav != NULL){
		if((*trav)->qty > (*trav)->delQty){
			...
			printf("\nAll of your %s have been delivered. We cannot cancel your order.", (*trav)->order.mealName);
		}
		checkOrderCompletion(MO);
	}
